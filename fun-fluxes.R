...
fitting.flux_nocut2 <- function(data,
  ...
  tz_df <- CO2_df %>% 
    ...
    mutate(
      CO2_roll = rollmean(CO2, k = roll_width, fill = NA, align = "right"),
      Cd = abs(CO2_roll - Cz),
  ...
  Cb_df <- CO2_df %>% 
    ...
    group_by(fluxID) %>% 
    mutate(
      Cb = CO2[time == tz_est - b_window] # what if tz_est < b_window?
    ) %>% 
    ungroup() %>% 
    select(fluxID, Cb) %>% 
    distinct()
  ...
  fitting_par <- CO2_df %>% 
    ...
    select(fluxID, Cm_est, a_est, b_est, tz_est, Cz, time, CO2) %>%
    ...
    summarize(
      ...
      results = list(optim(par = c(Cm_est, a_est, b_est, log(tz_est)), fn = myfn, CO2 = data$CO2, time = data$time, Cz = Cz)), #, lower=c(0, -Inf, -Inf, 0),  method="L-BFGS-B"
      Cm = results$par[1],
      a = results$par[2],
...
LRC.calc <- function(
    lrc_df, # df with flux values for all the LRC measurements
    fluxes_df, # df with all the fluxes
    group = c("siteID", "OTC"), # variables to group the lrc by
    PARfix = 300, # PAR value to correct NEE fluxes to
    PARnull = 0 # PAR value to correct ER fluxes to
    )
